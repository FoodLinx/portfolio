    ...
    switch (req.method){
        case 'POST': {
            try {
                ...
                const newRegistration = await Resturants.create({owner:owner, name:name, email:email, address:address, contact:contact, status:false})
                const newNotification = await Registrations.create({type:'business', identifier:newRegistration._id, name:name})
